    ...
    items: [
      {
        ...
        icon: Bell,
        children: ["Todas", "No leídas", "Tareas", "Aprobaciones", "Menciones"],
      },
      {
        label: "Configuración",
        icon: Settings,
  ...
  return (
    <div className="mx-auto max-w-[1440px] px-5 py-7 lg:px-9">
      <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
        ...
        <button
          onClick={() => onAction("Nuevo documento preparado")}
      ...
      <div className="mt-7 grid gap-6 xl:grid-cols-[1fr_300px]">
        <div className="rounded-2xl border border-[#dcebe8] bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-[#edf3f1] p-5">
            ...
            <button className="text-xs font-semibold text-[#0f9d9a]">
              Ver todos
            </button>
          </div>
        ...
        <div className="flex flex-col gap-6">
          <div className="rounded-2xl border border-[#dcebe8] bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 flex flex-col gap-2">
              {[
                [FilePlus2, "Nuevo documento", "Crear desde una plantilla"],
                [Upload, "Subir archivo", "Añadir un archivo al sistema"],
                [Workflow, "Crear workflow", "Diseñar un flujo de trabajo"],
              ].map(([Icon, label, hint]) => (
                <button
                  key={label as string}
                  onClick={() => onAction(`${label} preparado`)}
...
function UserTable({ onAction }: { onAction: (message: string) => void }) {
  return (
    <TableShell
...
function TenantTable({ onAction }: { onAction: (message: string) => void }) {
  return (
    <TableShell
...
function WorkflowTable({ onAction }: { onAction: (message: string) => void }) {
  return (
    <TableShell
...
export default function Home() {
  ...
  const [activeSubcategory, setActiveSubcategory] = useState("Resumen");
  const [expanded, setExpanded] = useState<string[]>(["Inicio"]);
  ...
  const selectNav = (item: NavItem, child?: string) => {
    setActiveNav(item.label);
    setActiveSubcategory(child ?? item.children[0]);
    setExpanded((current) =>
  ...
  return (
    <main className="min-h-screen bg-[#f6faf9] text-[#153a39]">
      ...
      <section className="lg:pl-[252px]">
          ...
          <div className="ml-auto flex items-center gap-3">
            <button
              onClick={() => {
                ...
                setActiveSubcategory("No leídas");
              }}
        ...
        {activeNav === "Inicio" && activeSubcategory === "Resumen" ? (
          <Dashboard onAction={notify} />
        ) : (
          <ModuleContent
            module={activeNav}
              ...
              return (
                <div key={item.label}>
                  ...
                  >
                    ...
                    <ChevronDown
                      className={`ml-auto size-3.5 transition-transform ${isExpanded ? "rotate-180" : ""}`}
                    />
                  </button>
                  {isExpanded && (
                    <div
                    ...
                    >
                      {item.children.map((child) => (
                        <button
                          key={child}
                          onClick={() => onSelect(item, child)}
                          className={`rounded-lg px-2 py-1.5 text-left text-[11px] transition ${activeSubcategory === child && isActive ? (mobile ? "bg-[#eafaf7] font-semibold text-[#087f7b]" : "bg-teal-100/15 font-semibold text-white") : mobile ? "text-[#78918f] hover:bg-[#f4faf9]" : "text-teal-100/75 hover:bg-white/10 hover:text-white"}`}
                        >
                          {child}
                        </button>
                      ))}
